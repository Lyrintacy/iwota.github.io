...
export default function Header(props) {
  ...
  var onMapToggle = props.onMapToggle;
  var showMap = props.showMap;
  ...
  return React.createElement('div', null,
    React.createElement('div', {
        ...
        React.createElement('div', null,
          React.createElement('div', {
            style: {
              ...
              color: '#fff', fontFamily: config.contentFont, lineHeight: 1.1,
            }
          }, config.name + ' ' + config.surname),
          !isMobile && section ? React.createElement('div', {
            style: {
              fontSize: 9, color: sColor + '90', fontFamily: config.uiFont,
      ...
      // Right: contact + map toggle (mobile)
      React.createElement('div', {
        style: { display: 'flex', alignItems: 'center', gap: isMobile ? 6 : 10 }
      },
        // Contact button
        ...
        isMobile ? React.createElement('button', {
          ...
          style: {
            ...
            background: showMap ? sColor + '25' : 'rgba(255,255,255,0.06)',
            border: '1px solid ' + (showMap ? sColor + '50' : 'rgba(255,255,255,0.1)'),
            color: showMap ? sColor : 'rgba(255,255,255,0.5)',
            fontSize: 16,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            transition: 'all 0.15s',
            flexShrink: 0,
          ...
          // Three lines icon (hamburger)
          React.createElement('div', {
            style: { display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'center' }
          },
            React.createElement('div', { style: {
              width: 14, height: 1.5, borderRadius: 1,
              background: showMap ? sColor : 'rgba(255,255,255,0.5)',
              transition: 'all 0.15s',
              transform: showMap ? 'rotate(45deg) translate(2px, 2px)' : 'none',
            }}),
            React.createElement('div', { style: {
              width: 14, height: 1.5, borderRadius: 1,
              background: showMap ? sColor : 'rgba(255,255,255,0.5)',
    ...
    contactOpen ? React.createElement(ContactModal, {
      sColor: sColor, isMobile: isMobile,
...
function ContactModal(props) {
  ...
  var handleSend = function() {
    ...
    var data = {
      ...
      message: message, from_name: name, replyto: email,
    };
  ...
  var handleKeyDown = function(e) { e.stopPropagation(); if (e.key === 'Escape') onClose(); };

  var inp = {
    width: '100%', padding: '10px 12px',
    background: 'rgba(255,255,255,0.05)',
    border: '1px solid rgba(255,255,255,0.1)',
    borderRadius: 6, color: '#ddd', fontSize: 13,
    fontFamily: "'Inter', 'Outfit', sans-serif",
  };
  var labelStyle = {
    fontSize: 10, color: 'rgba(255,255,255,0.5)',
    fontFamily: config.uiFont, textTransform: 'uppercase',
  ...
  if (status === 'sent') {
    return React.createElement('div', {
      ...
      React.createElement('div', {
        style: { width: isMobile ? '85%' : 380, background: 'rgba(15,15,28,0.98)',
          border: '1px solid ' + sColor + '30', borderRadius: 12,
          padding: isMobile ? 24 : 32, textAlign: 'center' }
      },
        React.createElement('div', { style: {
          width: 50, height: 50, borderRadius: 25, background: sColor + '20',
          border: '2px solid ' + sColor + '50', display: 'flex',
          alignItems: 'center', justifyContent: 'center',
          margin: '0 auto 16px', fontSize: 22, color: sColor,
        }}, '\u2713'),
        React.createElement('div', { style: { fontSize: 20, color: '#fff',
          fontFamily: config.contentFont, fontWeight: 700, marginBottom: 8 }
        }, 'Message Sent!'),
        React.createElement('div', { style: { fontSize: 11, color: 'rgba(255,255,255,0.4)',
          fontFamily: config.uiFont }
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      style: { width: isMobile ? '90%' : 400, maxHeight: '90vh', overflowY: 'auto',
        background: 'rgba(15,15,28,0.98)', border: '1px solid ' + sColor + '30',
        borderRadius: 12, padding: isMobile ? 20 : 28 }
    },
      ...
      error ? React.createElement('div', { style: {
        ...
        color: '#ff6666', fontSize: 11, fontFamily: config.uiFont, marginBottom: 14 }
      }, error) : null,

      React.createElement('div', { style: { marginBottom: 14 } },
        React.createElement('label', { style: labelStyle }, 'Your Name'),
        React.createElement('input', { value: name, onChange: function(e) { setName(e.target.value); setError(''); },
          onKeyDown: handleKeyDown, placeholder: 'John Doe', style: inp })
      ),
      React.createElement('div', { style: { marginBottom: 14 } },
        React.createElement('label', { style: labelStyle }, 'Your Email'),
        React.createElement('input', { value: email, onChange: function(e) { setEmail(e.target.value); setError(''); },
          onKeyDown: handleKeyDown, placeholder: 'john@example.com', type: 'email', style: inp })
      ),
      React.createElement('div', { style: { marginBottom: 14 } },
        ...
        React.createElement('input', { value: subject, onChange: function(e) { setSubject(e.target.value); },
          onKeyDown: handleKeyDown, placeholder: 'Project inquiry', style: inp })
      ),
      React.createElement('div', { style: { marginBottom: 18 } },
        React.createElement('label', { style: labelStyle }, 'Message'),
        React.createElement('textarea', { value: message, onChange: function(e) { setMessage(e.target.value); setError(''); },
          onKeyDown: handleKeyDown, placeholder: 'Tell me about your project...', rows: 4,
      ...
      React.createElement('button', {
        ...
        style: { width: '100%', padding: '12px',
          ...
          borderRadius: 6, color: status === 'sending' ? '#555' : sColor,
          fontSize: 12, fontFamily: config.uiFont, fontWeight: 700,
